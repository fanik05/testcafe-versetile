...
import onboardingModel from '../page_models/onboarding';
import { validEmail, validPassword, lockedEmail, lockedPassword, invalidEmail, invalidPassword } from '../user_credentials/dev_credentials';
...
const lockedUser = Role('https://member.dev.clubswan.com/', async t => {
    await t
        .typeText(loginModel.emailField, lockedEmail)
        .typeText(loginModel.passwordField, lockedPassword)
        .click(loginModel.loginButton)
        .expect(getPageUrl()).eql('https://member.dev.clubswan.com/onboarding/lock-account')
        .expect(onboardingModel.errorText.innerText).eql('We couldn\'t process your last payment.');
    await onboardingModel.doThisLater();
    await t
        .expect(getPageUrl()).eql('https://member.dev.clubswan.com/dashboard')
        .expect(dashboardModel.welcomeBanner.innerText).contains('Welcome to Club Swan');
});
...
test
    .meta('testID', 't-0009')
    ('Check if locked user can see payment due message and pressing do this later redirects to dashboard', async t => {
        await t
            .useRole(lockedUser);
    });    
